...
gui n =  do
  ...
  b1 <- button f [ text := " ", bgcolor  := yellow  ]
  b2 <- button f [ text := " " , bgcolor  := green ]
  b3 <- button f [ text := " " , bgcolor  := red ]
  b4 <- button f [ text := " " , bgcolor  := blue ]
  ...
  set b4 [ on command := set b3 [ bgcolor  := white ] ]
